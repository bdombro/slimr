...
Use it when your app is local-only, when you disable sync in certain environments, or when you **swap adapters in one app** (e.g. demo) and want the same `onLogin` / `bootstrapSession` / `login` / `logout` flow as `RestAdapter`.
...
- **Session APIs on** — `bootstrapSession()`, `login()`, `logout()`, `onLogin` / `onLogout`, and cross-tab auth behave like a REST app; the adapter **stubs** `checkAuth()` / `sendCode()` / `login()` / `logout()` / sync.

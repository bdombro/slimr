...
With `requiresAuth` adapters (like `RestCookieAdapter` / `RestBearerAdapter`), also ensure `db.auth.isLoggedIn` before reads/writes, or data APIs throw `DbSyncNotAuthenticatedError`.
...
With `requiresAuth` adapters (default for `RestCookieAdapter` / `RestBearerAdapter`), data APIs and `getTransaction()` throw `DbSyncNotAuthenticatedError` when `!db.auth.isLoggedIn`. See [Offline.md](./Offline.md) and [Errors](./Errors.md).

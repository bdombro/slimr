...
/** Resolves during the next idle period (via requestIdleCallback), or falls back to setTimeout. */
export function yieldToIdle() {
	return new Promise<void>((resolve) => {
		if (typeof (globalThis as any).requestIdleCallback === "function") {
			// Provide a timeout so it doesn't stall forever if the main thread is extremely busy

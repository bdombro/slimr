 ...
 * }
 */
export function templateStrPropsToStr(strings: TemplateStringsArray, ...placeHolders: string[]) {
	return strings.map((s, i) => s + (placeHolders?.[i] ?? '')).join('')
}
export type TemplateStringProps = [strings: TemplateStringsArray, ...placeHolders: string[]]
